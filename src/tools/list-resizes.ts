...
export const listResizesGram = new Gram().tool({
  name: "list_resizes",
  description:
    "Vitess/MySQL databases only. List recent resize operations for a PlanetScale database branch. Returns VTGate (connection proxy) resizes, keyspace/VTTablet (storage compute) resizes, and individual shard resizes. Automatically discovers keyspaces and fetches per-keyspace resize history. Each resize entry shows what changed with before/after values, the current state, and who initiated it. To see current shard sizes instead of resize history, use list_custom_shards.",
  inputSchema: {
