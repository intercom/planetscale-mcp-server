...
export const getBranchKeyspacesGram = new Gram().tool({
  ...
  description:
    "Vitess/MySQL databases only. List keyspaces for a PlanetScale database branch. Returns keyspace configuration including shard count, default cluster size, replica count, replication durability settings, and MySQL/VTTablet options. Useful for understanding the topology and configuration of a database. Note: the cluster size shown is the keyspace-level default — individual shards may have been resized independently. To see actual deployed sizes per shard, use list_custom_shards.",
  inputSchema: {
