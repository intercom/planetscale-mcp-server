...
export const getInfrastructureGram = new Gram().tool({
  ...
  description:
    "Vitess/MySQL databases only. Requires OAuth2 authentication (service tokens will return 404). Get the actual deployed infrastructure for a PlanetScale database branch. Returns VTGate sizes, per-shard cluster sizes (hardware SKUs), tablet placement across availability zones, and keyspace state. For shard sizing only, prefer list_custom_shards which is more efficient and works with service tokens. When called without a shard parameter, returns infrastructure for the default shard only — to get a specific shard's hardware, pass the shard parameter. To get ALL shard sizes, set all_shards=true (makes one API call per shard).",
  inputSchema: {
